...
bool tracer_msgs__msg__tracer_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // motor_states
    ...
    Py_ssize_t size = 2;
    tracer_msgs__msg__TracerMotorState * dest = ros_message->motor_states;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!tracer_msgs__msg__tracer_motor_state__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * tracer_msgs__msg__tracer_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // motor_states
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = tracer_msgs__msg__tracer_motor_state__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
